...
class DateTimeRangePickerViewModel(private val timeFormatter: TimeFormatter) {
    ...
    internal val minDate: BehaviorSubject<Date> = BehaviorSubject.create();
    internal val maxDate: BehaviorSubject<Date> = BehaviorSubject.create();
    ...
    fun handleArgs(arguments: Bundle) {
        ...
        if (arguments.containsKey(KEY_CALENDAR_MIN_TIME_IN_MILLIS)) {
            val startInMillis = arguments.getLong(KEY_CALENDAR_MIN_TIME_IN_MILLIS)
            minDate.onNext(DateTime(startInMillis, DateTimeZone.forTimeZone(timeZone)).toDate())
        } else {
            minDate.onNext(DateTime.now(DateTimeZone.forTimeZone(timeZone)).toDate())
        }

        if (arguments.containsKey(KEY_CALENDAR_MAX_TIME_IN_MILLIS)) {
            val endInMillis = arguments.getLong(KEY_CALENDAR_MAX_TIME_IN_MILLIS)
            maxDate.onNext(DateTime(endInMillis, DateTimeZone.forTimeZone(timeZone)).toDate())
        } else {
            maxDate.onNext(DateTime.now(DateTimeZone.forTimeZone(timeZone)).plusYears(1).toDate())
        }
    }
    ...
    companion object {
        ...
        val KEY_CALENDAR_MIN_TIME_IN_MILLIS = "calendarMinTimeInMillis"
        val KEY_CALENDAR_MAX_TIME_IN_MILLIS = "calendarMaxTimeInMillis"
